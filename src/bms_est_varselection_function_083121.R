...
est_varselection_bms=function( datamat, Xmat,true.mod,bootn=100){
  ...
  for(i in 1:length(truecoef)){
    if(rownames(truecoef)[i] %in% rownames(as.data.frame(true.mod$coefficients))){
      truecoef[i]=true.mod$coefficients[match(rownames(truecoef)[i],
                                              rownames(as.data.frame(true.mod$coefficients)))]
    }
  }
  ...
  for (b in 1:bootn){
    ...
    comp_time_fit[b,1] <- system.time({EBlocal.mod <- bas.lm(Y~.,sampdata, prior = "EB-local",method = "MCMC", MCMC.iterations = 10000)})[3]
    ...
    comp_time_fit[b,2] <- system.time({gsqrtn.mod <- bas.lm(Y~., sampdata, prior = "g-prior", alpha = sqrt(N),method = "MCMC", MCMC.iterations = 10000)})[3]
    ...
    comp_time_fit[b,3] <- system.time({hypergprior.mod <- bas.lm(Y~.,sampdata, prior = "hyper-g",method = "MCMC", MCMC.iterations = 10000)})[3]
    ...
    confscore.matrix=data.frame(EBlocal.ind[-1],
                                gsqrtn.ind[-1],
                                hypergprior.ind[-1],
                                JZS.probne0[-1])    #confscore.matrix=confscore.matrix[-1, ]
  ...
  results <- list("mse_mat_coef"=mse_mat_coef, "mae_mat_coef"=mae_mat_coef,
                  ...
                  "AUPRC.mat"=auprc.mat, "AUROC.mat"=auroc.mat, 
                  "zero_coverage"=colMeans(zero_coverage),
                  "nzero_coverage"=colMeans(nzero_coverage), 
                  "zero_IntScore"=colMeans(zero_intscore),
