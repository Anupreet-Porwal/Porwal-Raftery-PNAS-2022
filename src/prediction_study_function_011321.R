...
prediction_study=function(Xmat,Y, bootn=100, split=0.75){
  ...
  for (b in 1:bootn){
    ...
    a=0.05
      ...
      #Bicreg BMA
      bma.mod <- bicreg(X.samp[ ,-1], Y.samp, strict = FALSE, OR=100,maxCol =100)
      bma.pred <- predict(bma.mod,test.samp[ ,-1 ], quantiles=c(0.025,0.975))
      bma.pred.mat <- post.samples(2500, bma.mod, test.samp)
      ...
      gpriorUnit.mod <- bas.lm(Y~., train, prior = "g-prior", alpha = N.train,method = "MCMC",MCMC.iterations = 10000)
      ...
      #BIC g prior
      gpriorBIC.mod <- bas.lm(Y~., train, prior = "BIC",method = "MCMC",MCMC.iterations = 10000)
      ...
      #AIC
      AIC.mod <- bas.lm(Y~., train, prior = "AIC",method = "MCMC",MCMC.iterations = 10000)
      ...
      #EB-global
      EBglobal.mod <- bas.lm(Y~., train, prior = "EB-global",method = "MCMC",MCMC.iterations = 10000)
      ...
      #g = 1
      g1.mod <- bas.lm(Y~., train, prior = "g-prior", alpha = 1,method = "MCMC",MCMC.iterations = 10000)
      ...
      beta.init <- rep(1,p)
      sigma.init <- 1
      v0 <- seq(0.001,2,length.out=1000)
      v1 <- 1000
      ...
      # No sure how to calculate; PIP specially bad for wide datasets
      ...
      pred.mat <- list(bma.pred$mean,rowMeans(ss.pred), gpriorUnit.pred$fit,gpriorBIC.pred$fit,
      ...
      conf.mat <- list(bma.pred$quantiles, ss.conf, gpriorUnit.conf[ ,1:2], gpriorbic.conf[ ,1:2],
